...
| H-F1.4 | 🟡 `.env.local` con todas las variables. **REABIERTO 2026-06-16:** en el equipo de trabajo nuevo (`E:\2CLAUDE\Archibots`) NO existe `.env.local`; falta en particular `VITE_GOOGLE_MAPS_API_KEY` (por eso el mapa no carga en `npm run dev`). Recrearlo a partir de `.env.local.example`. | Local / repo | 🟡 PENDIENTE (equipo nuevo) |
...
```bash
# Desde C:\Users\gcastillo\Desktop\Archibots\Archibots
...
**Carpeta objetivo:** `C:\Users\gcastillo\Desktop\Archibots\Archibots\src\tools\`
