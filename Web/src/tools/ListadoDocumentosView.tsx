...
/* ── constantes ────────────────────────────────────────────────────────────── */
const TOOL_ID = 'listado-dom';
const LISTADO_VACIO: ListadoGuardado = { projectType: 'obra_nueva', selected: obligatoriosDe('obra_nueva'), customDocs: [] };
...
export default function ListadoDocumentosView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const { data, save, loading } = useToolData<ListadoGuardado>(TOOL_ID, projectId, LISTADO_VACIO);

  /* ── carga inicial (una sola vez) desde useToolData; el Set se reconstruye aquí ── */
  useEffect(() => {
    if (loading || hidratado) return;
    setProjectType(data.projectType);
    setSelected(new Set(data.selected));
    setCustomDocs(data.customDocs);
    setHidratado(true);
  }, [loading, hidratado, data]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      triggerToast(ok ? 'Listado de documentos guardado.' : 'No se pudo guardar el listado.');
    } catch {
