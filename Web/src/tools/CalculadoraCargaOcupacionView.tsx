...
/* ── constantes ────────────────────────────────────────────────────────────── */
const TOOL_ID = 'carga-ocupacion';
const CARGA_VACIA: CargaGuardada = { sectores: [
...
export default function CalculadoraCargaOcupacionView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const { data, setData, save } = useToolData<CargaGuardada>(TOOL_ID, projectId, CARGA_VACIA);
  const { sectores } = data;
  const setSectores = (u: SetStateAction<Sector[]>) => setData((d) => ({ ...d, sectores: typeof u === 'function' ? u(d.sectores) : u }));
  ...
  const handleSave = async () => {
    ...
    try {
      const ok = await save();
      triggerToast(ok ? 'Carga de ocupación guardada.' : 'No se pudo guardar la carga de ocupación.');
    } catch {
