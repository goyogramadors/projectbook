...
/* ── constantes ────────────────────────────────────────────────────────────── */
const TOOL_ID = 'estados-pago';
const ESTADO_VACIO: EstadoPagoGuardado = { glosa: 'Honorarios de Arquitectura - Fase 1', valor: '1000000', impuesto: '15.25', moneda: 'CLP' };
...
export default function EmisorEstadoPagoView({ projectId, access = 'edit' }: ToolProps) {
  ...
  // Persistencia unificada (Fase 2): carga y guardado vía useToolData (nube Premium /
  // localStorage Free). Aliases de lectura/escritura para no alterar el resto del JSX.
  const { data, setData, save } = useToolData<EstadoPagoGuardado>(TOOL_ID, projectId, ESTADO_VACIO);
  const { glosa, valor, impuesto, moneda } = data;
  const setGlosa = (v: string) => setData((d) => ({ ...d, glosa: v }));
  const setValor = (v: string) => setData((d) => ({ ...d, valor: v }));
  const setImpuesto = (v: string) => setData((d) => ({ ...d, impuesto: v }));
  const setMoneda = (v: Moneda) => setData((d) => ({ ...d, moneda: v }));
  ...
  const handleSave = async () => {
    ...
    try {
      const ok = await save();
      triggerToast(ok ? 'Estado de pago guardado.' : 'No se pudo guardar el estado de pago.');
    } catch {
