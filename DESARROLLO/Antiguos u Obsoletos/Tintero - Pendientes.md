...
## ❓ Auditoría de persistencia: ¿qué se guarda solo en localStorage y no en la BD? (2026-06-26)

**Contexto / pregunta abierta (Gregorio):** "¿qué significa que se guarda solo en localStorage?".

**Respuesta corta:** `localStorage` es el almacenamiento **del navegador en ese computador** (no es un servidor ni la nube). Es por-navegador y por-dispositivo: si cambias de equipo o limpias el navegador, esos datos no están. NO es la base de datos (Firestore).

**Modelo actual (recordatorio):**
- **Premium** → datos en **Firestore** (nube, sincronizado entre dispositivos).
- **Free** → datos en **localStorage** (solo ese navegador). Además, si la nube falla, varias rutas **degradan a localStorage** para no perder el trabajo.
- Preferencias de UI (tema, override de producto) viven en localStorage a propósito (correcto).

**⚠️ Datos que hoy se escriben SOLO en localStorage (no van a Firestore ni siquiera en Premium):**
- `tools/UbicacionView.tsx` → `STORAGE_KEY` (región antigua) y `TERRENO_KEY` (polígono/área del terreno).
- `tools/MapaTerrenoView.tsx` y `tools/GeolocalizadorView.tsx` → `TERRENO_KEY` (ring + área del terreno).
- `tools/SeguimientoObrasView.tsx` → `STORAGE_KEY` (minutas/seguimiento de visitas).
- `tools/VolumenTeoricoView.tsx` → `STORAGE_KEY` (inputs del volumen teórico).
- `tools/ExpedienteMunicipalView.tsx` → `STORAGE_KEY` (estado del expediente).
- Nota: el master del proyecto Free (`ProjectRepository` LOCAL_KEY) y los datos de herramientas Free (`useToolData`) también son locales **por diseño del tier**.

**Pendiente a trabajar:** decidir cuáles de esos stores tool-específicos deben migrar a Firestore (vía `useToolData` o subcolección) para que Premium los sincronice.

**Resuelto (2026-06-26):**
- [x] **Terreno (polígono/área)** → migrado a nube vía `src/tools/terrenoStore.ts` (Premium → `projects/{pid}/toolData/terreno`, regla `toolData/{document=**}` existente; espeja a localStorage). Cableado en Ubicación, Mapa de Terreno y Geolocalizador.
- [x] **Seguimiento de Obras** → ya persistía en Firestore para Premium (`SeguimientoObrasView.tsx`); la nota original estaba desactualizada. No requirió cambios.
- [x] **Volumen Teórico (Cabida)** → ya persistía en Firestore para Premium (`VolumenTeoricoView.tsx`, subcolección `projects/{id}/volumen/estado`); la nota estaba desactualizada. No requirió cambios.
- [x] **Expediente Municipal (DOM)** → migrado a nube vía `useToolData('expediente-dom')` (Premium → `projects/{pid}/toolData/expediente-dom`, regla `toolData/{document=**}`; Free/offline → localStorage `ab-expediente-dom-{pid}`, misma clave: datos previos conservados). Guardado con debounce (600 ms).

**Conclusión:** todos los stores tool-específicos señalados en la auditoría ya sincronizan en la nube para Premium.
