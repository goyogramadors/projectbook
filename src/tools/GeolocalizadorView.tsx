...
export default function GeolocalizadorView({ projectId, access = 'edit' }: ToolProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const markerLib: MapsAny = await loader.importLibrary('marker');
        const geoLib: MapsAny = await loader.importLibrary('geocoding');
        ...
        map.addListener('click', (e: MapsAny) => { if (!readOnly && e.latLng) setPoint(e.latLng.lat(), e.latLng.lng()); });
        ...
        const dm: MapsAny = new drawingLib.DrawingManager({
          ...
          polygonOptions: { fillColor: '#D32F2F', fillOpacity: 0.15, strokeColor: '#D32F2F', strokeWeight: 2, editable: false },
        });
        dm.setMap(map);
        ...
        gmaps.maps.event.addListener(dm, 'polygoncomplete', async (poly: MapsAny) => {
          ...
          for (let i = 0; i < path.getLength(); i++) {
            const p = path.getAt(i); ring.push([p.lng(), p.lat()]); sumLat += p.lat(); sumLng += p.lng();
          }
          const n = path.getLength() || 1;
          setPoint(sumLat / n, sumLng / n, true); // centroide como punto de análisis
  ...
  const intersectar = async () => {
    ...
    try {
      ...
      setZona(zonaCod);
      setFicha(resuelto?.normativa ?? null);
      setEstado('ok');
      if (!resuelto?.normativa) setError(`Zona ${zonaCod ?? '—'} detectada, pero sin ficha en normativas_prc.`);
    } catch (err) {
      setEstado('sinDato');
