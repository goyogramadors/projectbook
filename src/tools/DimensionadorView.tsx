...
/* ── constantes ────────────────────────────────────────────────────────────── */
const STORAGE_KEY = (pid: string) => `ab-dimensionador-${pid}`;
...
export default function DimensionadorView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const [programa, setPrograma] = useState<RecintoItem[]>([]);
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        const saved = JSON.parse(raw) as ProgramaGuardado;
        setPrograma(saved.programa ?? []);
        setCirculacion(saved.circulacion ?? 20);
      } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem(STORAGE_KEY(project.id), JSON.stringify({ programa, circulacion } satisfies ProgramaGuardado));
