...
const STORAGE_KEY = (pid: string) => `ab-dim-publicos-${pid}`;
...
export default function DimensionadorPublicosView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const [staffCounts, setStaffCounts] = useState<StaffCounts>(STAFF_VACIO);
  const [publicData, setPublicData] = useState<PublicData>({ modules: 0, peakWaiters: 0 });
  const [buildingConfig, setBuildingConfig] = useState<BuildingConfig>({ typology: 'A', includeCorporateAreas: false });

  /* ── carga del estado guardado ── */
  useEffect(() => {
    if (!project) return;
    let alive = true;
    const aplicar = (d: Partial<EstadoGuardado>) => {
      if (d.staff) setStaffCounts({ ...STAFF_VACIO, ...d.staff });
      if (d.publicData) setPublicData(d.publicData);
      if (d.buildingConfig) setBuildingConfig(d.buildingConfig);
    };
    (async () => {
      if (isPremium) {
        try {
          const snap = await getDoc(estadoDoc(project.id));
          if (alive && snap.exists()) { aplicar(snap.data() as EstadoGuardado); return; }
        } catch { /* offline / reglas → localStorage */ }
      }
      const raw = localStorage.getItem(STORAGE_KEY(project.id));
      if (alive && raw) { try { aplicar(JSON.parse(raw) as EstadoGuardado); } catch { /* corrupto */ } }
    })();
    return () => { alive = false; };
  ...
  const persist = async () => {
    const payload: EstadoGuardado = { staff: staffCounts, publicData, buildingConfig };
    if (isPremium) await setDoc(estadoDoc(project!.id), payload, { merge: true });
    else localStorage.setItem(STORAGE_KEY(project!.id), JSON.stringify(payload));
  };
