...
/* ── constantes ────────────────────────────────────────────────────────────── */
const STORAGE_KEY = (pid: string) => `ab-calc-minvu-${pid}`;
...
export default function CalculadoraConstruccionesMinvuView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const [constructions, setConstructions] = useState<Construccion[]>([CONSTRUCCION_NUEVA('B')]);
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        const saved = JSON.parse(raw) as Construccion[];
        if (Array.isArray(saved) && saved.length) setConstructions(saved);
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    try {
      if (project) localStorage.setItem(STORAGE_KEY(project.id), JSON.stringify(constructions));
      triggerToast(project ? 'Presupuesto MINVU guardado en el proyecto.' : 'Selecciona un proyecto para persistir el cálculo.');
    } catch {
