...
/* ── constantes ────────────────────────────────────────────────────────────── */
const STORAGE_KEY = (pid: string) => `ab-cuadro-superficies-${pid}`;
...
export default function CuadroSuperficiesView({ projectId, access = 'edit' }: ToolProps) {
  ...
  const [supOcupacion, setSupOcupacion] = useState('');
  const [supPredio, setSupPredio] = useState('');
  const [sobreTerreno, setSobreTerreno] = useState<Piso[]>([{ id: 'st-1', util: '', comun: '' }]);
  const [subterraneo, setSubterraneo] = useState<Piso[]>([]);

  /* ── carga inicial desde localStorage (o semilla desde el master) ── */
  useEffect(() => {
    if (!project) return;
    const raw = localStorage.getItem(STORAGE_KEY(project.id));
    if (raw) {
      try {
        const s = JSON.parse(raw) as Partial<CuadroGuardado>;
        setSupOcupacion(s.supOcupacion ?? '');
        setSupPredio(s.supPredio ?? project.superficieTerrenoLegal ?? '');
        setShowComun(Boolean(s.showComun));
        setSobreTerreno(Array.isArray(s.sobreTerreno) && s.sobreTerreno.length ? s.sobreTerreno : [{ id: 'st-1', util: '', comun: '' }]);
        setSubterraneo(Array.isArray(s.subterraneo) ? s.subterraneo : []);
        return;
      } catch { /* datos corruptos — se usa la semilla del master */ }
    }
    setSupPredio(project.superficieTerrenoLegal ?? '');
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: CuadroGuardado = { supOcupacion, supPredio, showComun, sobreTerreno, subterraneo };
